...
def setup_aws():
    ...
    commands = [ "sudo apt-get update > /dev/null", "sudo apt-get install --yes python-pip > /dev/null", "sudo apt-get install --yes python-dev > /dev/null", "sudo apt-get install --yes libev-dev > /dev/null", "tar -xf csc326-project.tar.gz", "sudo pip install -r requirements.txt", "sudo nohup python frontend.py " + ip_http + " > /dev/null 2> frontend.err < /dev/null &"] # these commands will exec in series
